...
```{r}
top_egg_zygote_miR <- cpm_s_ranked %>% 
  mutate(top = case_when(
    miR %in% egg_top_10$miR &
      miR %in% zygote_top_10$miR ~ "top egg & zygote",
    miR %in% egg_top_10$miR &
  ...
  select(sequence, sample_type, miR, miR2, CPM, top) %>% 
  spread(sample_type, CPM) %>% 
  filter(top == "top egg & zygote") %>% 
  select(sequence, miR2, egg, zygote) %>% 
  arrange(-zygote) %>% 
  mutate(DNA = str_replace_all(sequence, "U", "T")) 

write_excel_csv(top_egg_zygote_miR, "top_egg_zygote_miRNA.csv")
top_egg_zygote_miR
```
