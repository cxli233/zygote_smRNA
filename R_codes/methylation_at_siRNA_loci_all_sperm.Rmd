...
sperm_meth_list <- list.files(path = "C:/Users/cxli9/Desktop/Li/PBI/PBI299 (Sundar Lab)/smRNA project/CLVS_zygote_GR_revision/full_MTRs_zes_2021_10_13/sperm_data/", 
                        pattern = "*.mtr", full.names = T) 

sperm_meth_data <- sapply(sperm_meth_list, read_delim, simplify = FALSE, 
                       delim = "\t", col_names = F, escape_double = F, trim_ws = T) %>% 
  bind_rows(.id = "id")

head(sperm_meth_data) 
```
...
```{r}
set.seed(666)
sperm_meth_data_nice <- sperm_meth_data %>%
  #group_by(id) %>% 
  #sample_n(10000) %>% 
  ungroup() %>% 
  filter(X7 >= 3) %>% 
   mutate(sample_ID = case_when(
    str_detect(id, "sperm1") ~ "sperm Li et al", 
    str_detect(id, "SRR8594858") ~ "sperm Kim et al",
    str_detect(id, "SRR10915151") ~ "sperm Zhou et al.1",
    str_detect(id, "SRR10915152") ~ "sperm Zhou et al.2"
  )) %>% 
...
```{r}
sperm_meth_data_ss <- sperm_meth_data_nice %>% 
  filter(is.na(X4) == F) %>% 
  ...
  mutate(paper = case_when(
    ...
    str_detect(sample_ID, "Kim") ~ "Kim et al.",
...
write_excel_csv(sperm_meth_data_ss, "sperm_meth_data_ss.csv")
```
...
```{r}
model_sperm_meth <- glm(X4 ~ sample_ID* context * locus, data = sperm_meth_data_nice %>% 
                        group_by(id) %>% 
                        sample_n(10000), 
                     family = quasibinomial(link = "logit")) 

anova(model_sperm_meth, test = "LRT")
```

```{r}
est_sperm_meth1 <- emmeans(model_sperm_meth, pairwise ~ sample_ID | locus | context)
est_sperm_meth2 <- emmeans(model_sperm_meth, pairwise ~ locus | sample_ID | context)
```

```{r}
sperm_meth_results <- multcomp::cld(est_sperm_meth1$emmeans, Letters = letters) %>% 
  as.data.frame() %>% 
...
sperm_meth_data_ss %>% 
  filter(context == "mCHH") %>%
  inner_join(sperm_meth_results, by = c("sample_ID", "locus", "context")) %>% 
...
ggsave("sperm_meth.svg", height = 4, width = 9)
ggsave("sperm_meth.png", height = 4, width = 9)
```
...
sperm_meth_data_ss %>% 
  ...
  geom_errorbar(aes(ymin = LL, ymax = LL), width = 0.3, size = 1) +
  geom_text(aes(label = grouping, y = Q3), 
  hjust = -0.3, size = 4, fontface = "bold", vjust = -0.2) +
  scale_fill_manual(values = c("#541352", "#2f9aa0", "#ffcf20")) + 
  labs(x = NULL, 
       y = "mCHG in WT sperm cell",
       fill = "methylome data") +
  theme_minimal() +
  theme(
    text = element_text(size = 14, face = "bold", colour = "black"),
...
ggsave("sperm_meth_CHG.svg", height = 4, width = 9)
ggsave("sperm_meth_CHG.png", height = 4, width = 9)
```
...
ggsave("sperm_meth_CG.svg", height = 4, width = 9)
ggsave("sperm_meth_CG.png", height = 4, width = 9)
```

# plot - ~ locus | sample 
pull out groupings 
```{r}
sperm_meth_results2 <- multcomp::cld(est_sperm_meth2$emmeans, Letters = letters) %>% 
  as.data.frame() %>% 
  mutate(grouping = str_remove_all(.group, " ")) 
...
sperm_meth_data_ss %>% 
  filter(context == "mCHH") %>%
  inner_join(sperm_meth_results2, by = c("sample_ID", "locus", "context")) %>% 
  mutate(locus2 = case_when(
    str_detect(locus, "embryo siRNA loci") ~ "*embryo siRNA loci",
    T ~ locus
  )) %>% 
...
ggsave("sperm_meth_CHH_by_sample.svg", height = 8, width = 5)
ggsave("sperm_meth_CHH_by_sample.png", height = 8, width = 5)
```
