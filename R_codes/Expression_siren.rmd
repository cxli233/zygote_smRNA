...
# load data
```{r message=FALSE}
siren_cov_list <- list.files("C:/Users/cxli9/Desktop/Li/PBI/PBI299 (Sundar Lab)/smRNA project/CLVS13/fovary_bed_cov/siren_cov/", 
...
# egg vs zygote
```{r}
siren_cov_clean_s_ez <- siren_cov_clean_s %>% 
  filter(sample_type == "egg" |
           sample_type == "zygote") %>% 
  filter(siren_type == "egg siren loci") %>% 
  select(X1, X2, X3, sample_type, RPM) %>% 
  spread(sample_type, RPM)

head(siren_cov_clean_s_ez)
```
```{r}
siren_cov_clean_s_ez %>% 
  filter(zygote == min(zygote)) %>% 
  nrow()

summary(siren_cov_clean_s_ez$zygote)
```
## correlation 
```{r}
test1 <- cor.test(siren_cov_clean_s_ez$egg, siren_cov_clean_s_ez$zygote)
test1$p.value
test1$estimate
```
## regression 
```{r}
model_egg_zygote <- lm(log(zygote) ~ log(egg) + 0 , siren_cov_clean_s_ez)
summary(model_egg_zygote)
```
```{r}
...
ggsave("siren_expression_maintain_in_zygote.svg", height = 5, width = 5)
ggsave("siren_expression_maintain_in_zygote.png", height = 5, width = 5)
```
...
siren_cov_clean_ss %>%
  ...
  geom_text(aes(label = group, y = UL), size = 5, fontface = "bold", vjust = -0.3) +
...
ggsave("siren_RPM.svg", height = 5.5, width = 10)
ggsave("siren_RPM.png", height = 5.5, width = 10)
```

# how ovary siren in ovary vs egg (low in egg) 

```{r}
head(siren_cov_clean_s)

siren_wide <- siren_cov_clean_s %>% 
  select(X1, X2, X3, RPM, sample_type, siren_type) %>% 
  spread(sample_type, RPM)
...
## scatter 
```{r}
model_ovary_siren_egg_ov <- lm(log(egg) ~ log(ovary) + 0, data = siren_wide %>% 
                                 filter(siren_type == "ovary siren loci"))

summary(model_ovary_siren_egg_ov)
```

```{r}
siren_wide %>% 
  filter(siren_type == "ovary siren loci") %>% 
  ggplot(aes(y = egg, x = ovary)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm") +
  scale_x_log10(breaks = c(1, 10, 100, 1000)) +
  scale_y_log10(breaks = c(1, 10, 100, 1000)) +
...
ggsave("siren_expression_ovary_vs_egg.svg", height = 5, width = 5)
ggsave("siren_expression_ovary_vs_egg.png", height = 5, width = 5)
```
