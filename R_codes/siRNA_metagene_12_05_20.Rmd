...
read_delim("C:/Users/cxli9/Desktop/Li/PBI/PBI299 (Sundar Lab)/smRNA project/CLVS13/metagene_coverage/blender-sperm-1_S30_L002_R1_001.fastq.gz.trimmed.dedup.fastq.gz.sorted.bam_MSU7_filtered_genes.gff_downstream.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE) 
    

```{r message=F, warning=F}
meta_list <- list.files(path = "C:/Users/cxli9/Desktop/Li/PBI/PBI299 (Sundar Lab)/smRNA project/CLVS13/metagene_coverage/", 
...
```
```{r}
total_siRNA_length <- read_delim("C:/Users/cxli9/Desktop/Li/PBI/PBI299 (Sundar Lab)/smRNA project/CLVS13/siRNA_lengths/lengths_summary.bam.sam.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE)
```
...
```{r}
model_meta <- lm(pro_24 ~ sample_type, data = stat_data)
anova(model_meta) 
```
...
head(anova_result)
```
zygote - egg = (366 - 304)/304 * 100% = 20%
ovary 9 -ovary 0 = (344 - 327)/327 * 100% = 5%  
...
ggsave(filename = "24_meta_gamate_ov.svg", height = 4, width = 4.7)
ggsave(filename = "24_meta_gamate_ov.png", height = 4, width = 4.7)
```
```{r}
meta_data %>% 
  filter(genotype == "WT") %>% 
  ...
  geom_text(data = anova_result %>% 
              mutate(location == "5' end"), aes(label = grouping, y = text.y), size = 5, 
            fontface = "bold") +
  annotate(geom = "errorbarh", xmin = 2, xmax = 3, y = 380, 
            size = 1, height = 5) +
  annotate(geom = "errorbarh", xmin = 4, xmax = 5, y = 420, 
           size = 1, height = 5) +
  annotate(geom = "text", x = 2.5, y= 380, 
           label = "NS", size = 4, fontface = "bold", hjust = -0.2) +
  annotate(geom = "text", x = 4.5, y= 420, 
           label = "3e-8", size = 4, fontface = "bold", hjust = -0.1) +
...
ggsave(filename = "24_meta_gamate_ov_bar.svg", height = 3.5, width = 5)
ggsave(filename = "24_meta_gamate_ov_bar.png", height = 3.5, width = 5)
```
