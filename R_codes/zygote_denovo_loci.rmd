...
pc$x %>% 
  ...
  #geom_text_repel(aes(label = locus_type), size = 3) +
  scale_fill_manual(values = c("white", "orange2","blue2", "seagreen", "grey20",
                                "pink3", "violetred4", "tomato1",
                               "coral4", "dodgerblue2", "grey80"),
                    limits = c("genes", "TIR", "embryo DRM2 target loci", "seedling siRNA loci",
                               "*embryo siRNA loci",
                               "Z loci - E loci", 
                               "zygote siRNA loci",
                               "egg siRNA loci",
                               "E loci - Z loci",
                               "sperm siRNA loci", "Gypsy")) +
...
legend.source <- get_legend(
  ...
  scale_fill_manual(values = c("white", "orange2","blue2", "seagreen", "grey20",
                                "pink3", "violetred4", "tomato1",
                               "coral4", "dodgerblue2", "grey80"),
                    limits = c("genes", "TIR", "embryo DRM2 targets", "seedling siRNA loci",
                               "*embryo siRNA loci",
                               "Z loci - E loci", 
                               "zygote siRNA loci",
                               "egg siRNA loci",
...
dist_PC1 <- gene_dist_PC %>% 
  ...
  annotate(geom = "text", label = "rho = -0.98\nP = 0",
...
length_PC2 <- gene_dist_PC %>% 
  ...
  geom_point(aes(fill = ID, color = type2), shape = 21, alpha = 0.8, size = 3) +
  annotate(geom = "text", label = "rho = 0.88\nP = 0.0007",
...
```{r}
cor_matrix_data <- cor(cor_data[, -c(1, 8)], method = "s") %>% 
  as.data.frame() %>% 
...
ggsave("cor_heatmap.svg", height = 3.8, width = 4.2)
ggsave("cor_heatmap.png", height = 3.8, width = 4.2)
```
...
mCHG_PC1 <- cor_data %>% 
  ...
  geom_point(aes(fill = ID), shape = 21, alpha = 0.8, size = 3, color = "white") +
  annotate(geom = "text", label = "rho = -0.79\nP = 0.048",
           size = 3.5, x = Inf, y = Inf, hjust = 1.1, vjust = 1.2,
           fontface = "bold") + 
  scale_fill_manual(values = c( "seagreen", "grey20",
                                "pink3", "violetred4", "tomato1",
...
mCG_PC1 <- cor_data %>% 
  ...
  geom_point(aes(fill = ID), shape = 21, alpha = 0.8, size = 3, color = "white") +
  annotate(geom = "text", label = "rho = -0.95\nP = 8e-4",
           size = 3.5, x = Inf, y = Inf, hjust = 1.1, vjust = 1.2,
           fontface = "bold") + 
  scale_fill_manual(values = c( "seagreen", "grey20",
                                "pink3", "violetred4", "tomato1",
...
bin50_data_select_cent2 <- bin50_data_select %>% 
  ...
  mutate(tag = paste(Chr, start, end, "-")) 
...
bin50_data_select_cent %>% 
  ...
  geom_hline(data = bin50_data_select_s %>% 
  filter(str_detect(locus, "intersect") == F) %>%
               mutate(locus = case_when(
    locus == "embryo siRNA loci" ~ "*embryo siRNA loci",
    locus == "embryo DRM2 target loci" ~ "embryo DRM2 targets",
    T ~ locus
       ...
       aes(yintercept = mean.RPM)) +
  scale_fill_manual(values = c("black", "orange2","blue2", "seagreen", "grey20",
                                "pink3", "violetred4", "tomato1",
                               "coral4", "dodgerblue2", "grey80"),
                    limits = c("genes", "TIR", "embryo DRM2 targets", "seedling siRNA loci",
                               "*embryo siRNA loci",
...
bin50_data_select_cent %>% 
  ...
  # geom_crossbar(data = bin50_data_select_cent_s %>% 
  #              mutate(locus = case_when(
  #   locus == "embryo siRNA loci" ~ "*embryo siRNA loci",
  #   locus == "embryo DRM2 target loci" ~ "embryo DRM2 targets",
  #   T ~ locus
  # )), color = "red3", alpha = 0.8,
  #            aes(y = median, ymax = median, ymin = median), width = 0.5) +
  ...
  annotate(geom = "text", label = "4e-10", y = 450, hjust = -0.1, x = 6, 
           size = 3.5, fontface = "bold") +
  annotate(geom = "text", label = "4e-13", y = 550, hjust = -0.1, x = 8, size = 3.5,
           fontface = "bold") +
  scale_fill_manual(values = c("white", "orange2","blue2", "seagreen", "grey20",
                                "pink3", "violetred4", "tomato1",
                               "coral4", "dodgerblue2", "grey80"),
